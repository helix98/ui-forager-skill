...
6. **Flag runtime CDN script injection before using it.** A few stashed/fetched components load a third-party library at runtime via `document.createElement('script')` pointed at a CDN, instead of a normal npm install + import. Treat that as a supply-chain consideration worth surfacing, not a neutral style choice — mention it explicitly and suggest installing the package normally instead when a real npm equivalent exists.
